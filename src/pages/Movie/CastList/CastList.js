    ...
    return (
        <div className='actor'>
            ...
            <div className='castList'>
                {cast.length > 0 && cast.slice(0, 5).map(item => <CastItem key={item.id} data={item} />)}
            </div>
        </div>
    )
}
...
const CastItem = ({ data }) => {
    ...
    return (
        <div className='castInfo'>
            <img src={profile_path ? `https://image.tmdb.org/t/p/original/${profile_path}` : NoImg} alt={name} />
