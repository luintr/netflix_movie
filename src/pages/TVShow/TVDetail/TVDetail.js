...
const TVDetail = () => {
  ...
  const { backdrop_path, name, created_by, vote_average, genres, overview, languages, poster_path, production_companies, seasons } = detailData

  const DirectorItem = () => {
    return created_by.slice(0, 3).map(item => (
      <div className='profileDirector' key={item.id}>
        <img src={item.profile_path ? `https://image.tmdb.org/t/p/original/${item.profile_path}` : NoImg} alt={name} />
  ...
  return (
    <div className='detailTV'>
      <div className='detailBackdrop'>
        <img src={backdrop_path ? `https://image.tmdb.org/t/p/original/${backdrop_path}` : NoImg} alt={name} />
        ...
        <div className='boxInfo'>
          ...
          <div className='lang'>
            Languages:{languages.map(text => <Tag key={text} text={text.toUpperCase()} />)}
          </div>
        </div>
      </div>
      ...
      <div className='container-fluid'>
        <div className='middleInfo'>
          <div className='right'>
            <img src={poster_path ? `https://image.tmdb.org/t/p/original/${poster_path}` : NoImg} alt={name} />
          </div>
          <div className='left'>
            ...
            <div className='infoItem production'>
              ...
              <div className='productionList'>
                {production_companies.slice(0, 2).map(item => <div className='profileProduction' key={item.id}>
                  <div className='productionImg'>
                    {item.logo_path ? <img src={`https://image.tmdb.org/t/p/original/${item.logo_path}`} alt={item.name} /> : <p>No Img</p>}
                  </div>
                  <p className='profileName'>{item.name}</p>
                </div>)}
              </div>
              ...
              {seasons.map(item =>
                <SwiperSlide key={item.id} >
                  <div className='seasonItem'>
                    <img src={item.poster_path ? `https://image.tmdb.org/t/p/original/${item.poster_path}` : NoImg} alt={item.name} />

                    <div className='box'>
                      <p className='title'>{item.name}</p>
                      <div className='seasonBtn'><InfoRoundedIcon /></div>
                    </div>
                  </div>
                </SwiperSlide>
              )}
